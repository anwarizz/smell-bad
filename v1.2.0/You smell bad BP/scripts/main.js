...
world.afterEvents.playerSpawn.subscribe(({ initialSpawn, player }) => {
  if (initialSpawn) {
    ...
    if (!player?.getDynamicProperty("cover_nose_item")) {
      player.runCommandAsync("function sobs_cover_nose_item").catch(() => {});
      player.setDynamicProperty("cover_nose_item", true)
    } 
...
system.runInterval(() => {
  for (const player of world.getPlayers()) {
    ...
    if (heldItem && heldItem.typeId == "sobs:cover_nose") {
      player.removeTag("cover")
      player.addTag("cover")
    } 

    else if (!heldItem || heldItem.typeId != "sobs:cover_nose") {
      player.removeTag("cover")
    }
  }
})
    ...
    for (const player of world.getPlayers()) {
      try {
        ...
        if (stink_lv <= 100 && stink_lvc == 1) {
          ...
          const cmd = `effect @e[family=!poison_counter,name=!${name},tag=!cover,r=5] poison 5 0`;
  ...
  try {
    ...
    for (const ent of smelled2Entities) {
      ...
      for (const player of nearbyPlayers) {
        ...
        if (player.hasTag("cover")) continue
...
world.afterEvents.itemUse.subscribe((event) => {
  ...
  // if (itemStack?.typeId == "sobs:cover_nose") {
  //   source.playAnimation("animation.player.cover_nose");
  // }

  // if (itemStack?.typeId == "minecraft:stick") {
  //   // const stink_lv = source.getDynamicProperty("stink_lv");
  //   // world.sendMessage(`Stink LV: ${stink_lv}`);
  //   source.addTag("I say iiiiiiii")
  // }
  ...
  //   world.sendMessage("tags: " + tags)
  //   world.sendMessage("tag: " + tags[0])
  // }
